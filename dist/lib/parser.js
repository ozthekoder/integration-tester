...
module.exports = function () {
  function Parser() {
    var val = arguments.length <= 0 || arguments[0] === undefined ? _constants.validation : arguments[0];
    ...
    value: function flatten(blueprint) {
      ...
      if (!blueprint.$skip) {
        var _ret = function () {
          ...
          var base = _this2.recursive.map(function (key) {
            if (blueprint[key]) {
          ...
          var flat = base;
          for (i = 0; i < repeat; i++) {
            flat = [].concat(_toConsumableArray(flat), _toConsumableArray(base));
          }
